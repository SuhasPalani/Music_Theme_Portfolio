  ...
  const projects = [
    {
      ...
      description:
        "Designed a serverless AI-driven system for automated app feedback analysis and actionable insights.",
      details: [
      ...
      accentColor: "teal",
      musicGenre: "Cyber Ambient",
      link: "https://github.com/SuhasPalani/ai-analytics-project",
      status: "Lambda Anthem",
    },
    {
      ...
      <div className="relative max-w-7xl mx-auto px-4">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-300 mb-8">
            A symphonic journey through digital compositions and code
            orchestrations
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-4 mb-12 justify-center">
          {filters.map((filter) => (
            ...
            >
              ...
              {activeFilter === filter.name && (
                <Star
                  className="relative text-yellow-300 animate-spin"
                  size={16}
                />
              )}
            </button>
          ...
          {filteredProjects.map((project, index) => (
            ...
            >
              ...
              <div className="relative h-56 overflow-hidden">
                ...
                {/* Animated glow */}
                <div
                  className={`absolute inset-0 bg-gradient-to-r ${project.color} opacity-0 group-hover:opacity-30 blur-xl transition-opacity duration-500`}
                ></div>

                {/* Record Player */}
                <div className="absolute inset-0 flex items-center justify-center">
                  <div
                    className={`relative w-32 h-32 rounded-full bg-gradient-to-br from-black to-gray-900 border-4 border-${
                      project.accentColor
                    }-500/30 transition-transform duration-700 ${
                      playingRecord === index
                        ? "animate-spin"
                  ...
                  >
                    ...
                    {/* Center label */}
                    <div
                      className={`absolute inset-0 flex items-center justify-center rounded-full bg-gradient-to-br from-${project.accentColor}-600 to-${project.accentColor}-800`}
                    >
                  ...
                  {/* Tonearm */}
                  <div
                    className={`absolute top-1/3 right-1/4 w-16 h-2 bg-gradient-to-r from-${
                      project.accentColor
                    }-600 to-${
                      project.accentColor
                    }-800 rounded-full origin-left transition-transform duration-500 ${
                  ...
                  <div className="absolute top-4 right-4 bg-black/70 backdrop-blur-sm px-3 py-1 rounded-full">
                    <span
                      className={`text-xs font-bold text-${project.accentColor}-400`}
                    >
                      {project.status}
                    </span>
                  </div>
                ...
                {hoveredProject === index && (
                  <>
                    <Sparkles
                      className="absolute top-4 left-4 text-yellow-300 animate-bounce"
                      size={16}
                    />
                    <Star
                      className="absolute bottom-4 right-8 text-pink-400 animate-spin"
                      size={14}
                    />
                    <Zap
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-6">
                  ...
                  <div
                    className={`inline-block px-3 py-1 rounded-full bg-gradient-to-r ${project.color} text-xs font-medium text-white`}
                  >
                    {project.musicGenre}
                  </div>
                </div>
                ...
                <div className="space-y-3 mb-6">
                  {project.details.slice(0, 2).map((detail, idx) => (
                    <div key={idx} className="flex items-start">
                      <div
                        className={`w-2 h-2 rounded-full bg-${project.accentColor}-500 mr-3 mt-2 animate-pulse`}
                      ></div>
        ...
        {filteredProjects.length === 0 && (
          <div className="text-center py-20">
            <div className="relative inline-block">
              <Music
                size={64}
                className="text-purple-400 animate-bounce mx-auto mb-4"
              />
              <Sparkles
            ...
            <p className="text-gray-400">
              This genre is still being composed... Stay tuned!
            </p>
          </div>
        )}
      </div>
